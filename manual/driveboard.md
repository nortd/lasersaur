...
[Index](index) -- Driveboard | [Wiring Guide](wiring)

**NOTE**: We have detailed instructions and schematics on the [Wiring Guide](wiring) page.

The DriveBoard is the main electronics board of the Lasersaur (successor of [LasaurShield](LasaurShield)). It interfaces all the electronics components and runs the [Lasersaur software](software). This makes the Lasersaur fully controllable from any modern web browser over the local network (ethernet, wifi). The software is modular (firmware, backend, frontend) and allows for simple hacking and modding. The board is z-axis ready but is typically only operated with the x- and y-axis stepper drivers and motors.
...
See [Lasersaur Software](software).
